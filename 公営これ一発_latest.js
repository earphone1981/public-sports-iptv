...
const HIM_API_REPO =
  `/repos/${HIM_OWNER}/${HIM_REPO}`;

const HIM_EPG_WORKFLOW =
  "update_freewifi_epg.yml";
...
async function dispatchPublicEPG(
  token
) {
...
async function dispatchPublicYouTube(
  token
) {
...
async function dispatchHimitsuEPG(
  token
) {
...
try {
  ...
  await uploadFile(
    ...
    `Update BOATRACE M3U v14.2 ${boat.date}`
  );
  ...
  const changed =
    await uploadFile(
      ...
      `Update public sports M3U v14.2 ${boat.date}`
    );
  ...
  let publicEpgDispatchOK =
    false;


  let himitsuEpgDispatchOK =
    false;
