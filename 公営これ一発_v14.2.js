...
async function getToken() {
  ...
  if (r === -1) throw new Error("Token入力中止");
...
async function uploadFile(path, content, token, message) {
  ...
  let sha = null;
  try {
    const current = await githubRequest(`${API_REPO}/contents/${encodedPath}?ref=${BRANCH}`, token);
    sha = current.sha ?? null;
    if (current.content) {
      const old = Data.fromBase64String(current.content.replace(/\n/g, "")).toRawString();
  ...
  if (sha) body.sha = sha;
  await githubRequest(`${API_REPO}/contents/${encodedPath}`, token, "PUT", body);
  return true;
}
...
function readEntries(text) {
  ...
  for (let i=0; i<lines.length; i++) {
    ...
    for (let j=i+1; j<lines.length; j++) {
      ...
      if (n.startsWith("#EXTVLCOPT:")) { options.push(n); continue; }
      if (!n.startsWith("#")) { url=n; break; }
    }
...
function dedupeYouTubeEntries(entries) {
  const result=[], seenIds=new Set(), seenStreams=new Set();
...
function findStreamUrl(data) {
  ...
  if (Array.isArray(data)) {
    for (const item of data) { const found=findStreamUrl(item); if (found) return found; }
    return null;
  }
  if (typeof data === "object") {
    ...
    for (const key of Object.keys(data)) {
      ...
      if (value && typeof value === "object") { const found=findStreamUrl(value); if (found) return found; }
    }
  }
  return null;
}
...
async function makeBoatM3U(existingPublic) {
  ...
  let text="#EXTM3U\n\n";
  let freshCount=0, keptCount=0, missingCount=0;
  const fresh=[], kept=[], missing=[];
  for (const [apiId,tvgId,display,venue,logoFile] of BOAT) {
    const api="https://playback.api.streaks.jp/v1/projects/cp-boatrace-prod/medias/ref:lm-br-"+
      apiId+"-tokyo-"+date+"?audio_only=false";
    let stream=null, sourceType="none";
    try {
      const req=new Request(api);
      req.headers={"User-Agent":UA,"Origin":"https://front.player.boatrace-cdn.jp","Referer":"https://front.player.boatrace-cdn.jp/"};
      req.timeoutInterval=15;
      const found=findStreamUrl(await req.loadJSON());
      if (found) { stream=found; sourceType="fresh"; }
    } catch(e) { console.warn(`${venue} 当日URL取得失敗`,e); }
    if (!stream && previousBoat.has(tvgId)) { stream=previousBoat.get(tvgId); sourceType="kept"; }
    if (!stream) { missingCount++; missing.push(venue); continue; }
    const logo=rawUrl(`public_sports_logos_github_43/boatrace/${logoFile}`);
    text += `#EXTINF:-1 tvg-id="${tvgId}" tvg-name="${display}" tvg-logo="${logo}" group-title="ボートレース",${display}\n${stream}\n\n`;
    if (sourceType==="fresh") { freshCount++; fresh.push(venue); }
    else { keptCount++; kept.push(venue); }
  }
...
async function loadPublicYouTube() {
  try {
    ...
    return {entries,loaded:true,sourceCount:source.length,removed:source.length-entries.length,error:null};
  } catch(e) {
    return {entries:[],loaded:false,sourceCount:0,removed:0,error:String(e)};
  }
}
...
async function makePublicM3U(boatText,publicYouTube) {
  ...
  for (const [tvg,tvgName,display,file,logoFile] of JRA) {
    out.push(`#EXTINF:-1 tvg-id="${tvg}" tvg-name="${tvgName}" tvg-logo="${rawUrl(logoFile)}" group-title="中央競馬",${display}`);
...
async function dispatchPublicEPG(token) {
  await githubRequest(`${API_REPO}/actions/workflows/update_epg_3days.yml/dispatches`,token,"POST",{ref:BRANCH});
}

async function dispatchPublicYouTube(token) {
  await githubRequest(`${API_REPO}/actions/workflows/youtube_namibia_live.yml/dispatches`,token,"POST",{ref:BRANCH});
}

async function dispatchHimitsuEPG(token) {
  await githubRequest(`${HIM_API_REPO}/actions/workflows/${HIM_EPG_WORKFLOW}/dispatches`,token,"POST",{ref:HIM_BRANCH});
}
...
async function backupSelfScript(token) {
  try {
    ...
    for (const fm of managers) {
      for (const name of candidates) {
        try {
          ...
          if (fm.isFileStoredIniCloud?.(path)) { try { await fm.downloadFileFromiCloud(path); } catch {} }
    ...
    await uploadFile("公営これ一発_v14.2.js",selfText,token,"Backup 公営これ一発 v14.2");
    await uploadFile("公営これ一発_latest.js",selfText,token,"Backup 公営これ一発 latest v14.2");
    return true;
  } catch(e) {
...
try {
  ...
  let existingPublic="";
  try { existingPublic=await getRaw("public_sports.m3u"); } catch(e) { console.warn("既存M3U取得失敗",e); }
  ...
  await uploadFile("boatrace_today.m3u",boat.text,token,`Update BOATRACE M3U v14.2 ${boat.date}`);
  const changed=await uploadFile("public_sports.m3u",publicM3U,token,`Update public sports M3U v14.2 ${boat.date}`);

  let youtubeDispatchOK=false, publicEpgDispatchOK=false, himitsuEpgDispatchOK=false;
  let himitsuError="";
  try { await dispatchPublicYouTube(token); youtubeDispatchOK=true; } catch(e) { console.warn("公営YouTube Actions起動失敗",e); }
  try { await dispatchPublicEPG(token); publicEpgDispatchOK=true; } catch(e) { console.warn("公営EPG Actions起動失敗",e); }
  try { await dispatchHimitsuEPG(token); himitsuEpgDispatchOK=true; } catch(e) { himitsuError=String(e); console.warn("himitsu EPG Actions起動失敗",e); }
  ...
  if (!himitsuEpgDispatchOK && himitsuError) msg += `\n\n⚠️ Free Wi-Fi EPG起動：GitHub Tokenに ajiousama/himitsu のActions権限が必要です`;
  if (boat.kept.length) msg += `\n\n📼 前回映像保持：\n${boat.kept.join(" / ")}`;
  if (boat.missing.length) msg += `\n\n⚠️ URL履歴なし：\n${boat.missing.join(" / ")}`;

  await show("✅ 公営これ一発 v14.2 完了",msg);
} catch(e) {
  console.error(e);
  await show("❌ 公営これ一発 v14.2 エラー",String(e));
}
